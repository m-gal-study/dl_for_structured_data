...
#%%
# load config gile
current_path = os.getcwd() + '\deploy_web'
...
print(pipeline1_filename)
print(pipeline2_filename)
...
# pipeline_path = get_path('pipelines') # original
# mnu
pipeline_path = os.getcwd() + '\pipelines'
...
# model_path = os.path.join(get_path('models'), model_filename) # original
model_path = os.path.join(models_path, model_filename) # mnu
...
def about():
    ...
    loaded_model = load_model(model_path)
    # loaded_model._make_predict_function() # need to be commented
    ...
    if pred[0][0] >= 0.5:
        predict_string = f"yes, delay predicted with probability of {pred[0][0] * 100: 2.2f}%"
    else:
        predict_string = f"no delay predicted with probability of {pred[0][0] * 100: 2.2f}%"
